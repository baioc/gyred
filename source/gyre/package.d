...
Several other features are "mashed up" in Gyre's design, coming from an extensive survey on compilers, portable IRs and the [UNCOL](https://www.osdata.com/topic/language/uncol.htm) solution to the [compiler construction problem](https://doi.org/10.1145/368892.368915).
...
$(LIST
    ...
    * Progressivity: Premature lowering decreases optimization potential, so the IR must be able to preserve "high-level" structure and information for as long as this is desirable.
    * Parsimony: The IR must not expand uncontrollably to accommodate new languages or machines. Instead, it ought to contain a well-defined "core" which is small, yet expressive.
    * Extensibility: The IR should be amenable to extensions, both from users and to its own specification. These must to be clearly signaled for the sake of compatibility. This principle complements the previous one.
)
+/
module gyre;
